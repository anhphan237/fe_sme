...
interface Props {
    ...
    isDashboard?: boolean;
    defaultValue?: 'today' | 'yesterday' | 'thisMonth' | 'lastMonth' | 'thisYear' | 'lastYear';
}
...
const predefinedRanges: Record<string, [Dayjs, Dayjs]> = {
    ...
    thisMonth: [dayjs().startOf('month').startOf('day'), dayjs().endOf('month').startOf('day')],
    lastMonth: [dayjs().subtract(1, 'month').startOf('month').startOf('day'), dayjs().subtract(1, 'month').endOf('month').startOf('day')],
    thisYear: [dayjs().startOf('year').startOf('day'), dayjs().endOf('year').startOf('day')],
    lastYear: [dayjs().subtract(1, 'year').startOf('year').startOf('day'), dayjs().subtract(1, 'year').endOf('year').startOf('day')],
};
const DateRangeFilter: React.FC<Props> = ({ onChange, isDashboard = false, defaultValue }) => {
    const { t } = useLocale();

    const initialValue = isDashboard ? 'today' : defaultValue;
    const initialRange = initialValue && initialValue in predefinedRanges ? predefinedRanges[initialValue] : [null, null];

    const [selectedOption, setSelectedOption] = useState<string | undefined>(initialValue);
    const [range, setRange] = useState<[Dayjs | null, Dayjs | null]>(initialRange as [Dayjs | null, Dayjs | null]);

    const quickOptions = [
        {
            label: t('dashboard.overview.time_range.today'),
            value: 'today',
        },
        {
            label: t('dashboard.overview.time_range.yesterday'),
            value: 'yesterday',
        },
