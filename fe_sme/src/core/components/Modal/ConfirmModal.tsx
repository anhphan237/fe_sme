...
export interface ConfirmModalHandles {
    open: (params?: { message?: React.ReactNode }) => Promise<{ code: number; message: string }>;
}
...
interface ConfirmModalProps extends Omit<BaseModalProps, 'onConfirm' | 'onCancel'> {
    message?: React.ReactNode;
}

const ConfirmModal = forwardRef<ConfirmModalHandles, ConfirmModalProps>((props, ref) => {
    const [isOpen, setIsOpen] = useState(false);
    const { execute, resolve, reject } = usePromiseHolder<{ code: number; message: string }>({
        defaultValue: { code: -1, message: 'unknown' },
    });
    const { t } = useLocale();
    const [message, setMessage] = useState<React.ReactNode>(props.message || '');
        ...
        >
            <div>
                ...
                <div className="flex justify-between mt-4">
                    <Button className="w-[calc(50%_-_8px)]" type="primary" onClick={handleConfirm}>
                        {t('global.confirm')}
                    </Button>
