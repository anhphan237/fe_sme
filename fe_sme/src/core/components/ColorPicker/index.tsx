...
const BaseColorPicker = ({ name, label, formItemProps, ...props }: BaseColorPickerProps) => {
    return (
        <BaseFormItem label={label} name={name} getValueFromEvent={(color: Color) => color.toHexString()} {...formItemProps}>
            <ColorPicker {...props} />
        </BaseFormItem>
    );
};
