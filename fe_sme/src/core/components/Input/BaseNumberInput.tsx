...
const BlankInputWrapper = ({ children }: { children: React.ReactNode }) => <>{children}</>;
const TooltipInputWrapper = ({ children, tooltipProps }: { children: React.ReactNode; tooltipProps?: TooltipProps }) => (
...
const BaseInputNumber = ({
    ...
    const moneyParser = (value: string | undefined) => value?.replace(/\$\s?|(,*)/g, '') || '';
        ...
        <InputWrapper tooltipProps={tooltipProps}>
            <BaseFormItem label={label} name={name} {...formItemProps}>
                <InputNumber
                    ...
                    {...props}
                    className="w-full"
                    formatter={isMoneyFormat || isNumberFormat ? moneyFormatter : props.formatter}
