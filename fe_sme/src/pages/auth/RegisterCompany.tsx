...
import { checkEmailExists, registerCompany } from '../../shared/api/auth'
...
function RegisterCompany() {
  ...
  const handleContinueFromEmail = async () => {
    ...
    try {
      ...
      if (exists) {
        setEmailExistsError('Email already exists. Please choose another email.')
        return
      }
      setStep(1)
    } catch {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-6">
          ...
          {step === 3 && (
            <div className="rounded-lg border border-[#e5e5e7] bg-[#fbfbfd] p-4">
              <h3 className="text-[15px] font-semibold text-[#1d1d1f]">Summary</h3>
              <dl className="mt-3 space-y-2 text-[13px]">
                <div className="flex justify-between gap-4">
                  <dt className="text-[#86868b]">Company</dt>
                  <dd className="font-medium text-[#1d1d1f]">{getValues('companyName')}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-[#86868b]">Tax code</dt>
                  <dd className="font-medium text-[#1d1d1f]">{getValues('taxCode')}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-[#86868b]">Address</dt>
                  <dd className="font-medium text-[#1d1d1f]">{getValues('address')}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-[#86868b]">Timezone</dt>
                  <dd className="font-medium text-[#1d1d1f]">{getValues('timezone')}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-[#86868b]">Admin</dt>
                  <dd className="font-medium text-[#1d1d1f]">{getValues('adminFullName')}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-[#86868b]">Email</dt>
                  <dd className="font-medium text-[#1d1d1f]">{getValues('adminUsername')}</dd>
                </div>
                {getValues('adminPhone') && (
                  <div className="flex justify-between gap-4">
                    <dt className="text-[#86868b]">Phone</dt>
                    <dd className="font-medium text-[#1d1d1f]">{getValues('adminPhone')}</dd>
                  </div>
                )}
              </dl>
            </div>
