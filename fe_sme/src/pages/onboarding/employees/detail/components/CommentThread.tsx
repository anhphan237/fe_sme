...
import { apiListComments, apiAddComment } from "@/api/onboarding/onboarding.api";
import { extractList } from "@/api/core/types";
import type { CommentResponse } from "@/interface/onboarding";

// ─── Local Hooks ──────────────────────────────────────────────────────────────

const useCommentsQuery = (taskId?: string) =>
  useQuery({
    queryKey: ["task-comments", taskId ?? ""],
    queryFn: () => apiListComments({ taskId: taskId! }),
...
const useAddComment = () => {
  ...
  return useMutation({
    mutationFn: (payload: { taskId: string; message: string }) =>
