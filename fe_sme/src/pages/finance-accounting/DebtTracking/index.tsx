import { apiSearchDebtTracking } from '@/api/finance-accounting.api';
...
import DebtTrackingTable from '@/components/table';
...
const { Column } = DebtTrackingTable;
...
const DebtTracking = () => {
    ...
    const { data } = useQuery({
        queryKey: ['getListDebtTracking', filter],
        queryFn: async () => {
            const res = await apiSearchDebtTracking(filter);
            return res;
        },
    });
        ...
        <div className="h-full flex flex-col p-4">
            <div className="flex justify-between items-center gap-2 mb-3 mx-2">
                <div className="flex gap-4 items-center ">
                    <div className="relative flex flex-col items-center p-2 min-w-[180px] bg-white border border-gray-200 rounded-lg shadow-sm">
                        <div className="px-2 py-1 bg-white shadow text-center font-medium rounded-lg mb-4 absolute top-[-15px] right-4 ">
                            <span className="text-xs whitespace-normal text-center ">{t('finance_accounting.totalAmount')}</span>
                        </div>
                    ...
                    <div className="relative flex flex-col items-center p-2 min-w-[180px] bg-white border border-gray-200 rounded-lg shadow-sm">
                        <div className="px-2 py-1 bg-white shadow text-center font-medium rounded-lg mb-4 absolute top-[-15px] right-4">
                            <span className="text-xs whitespace-normal text-center ">{t('finance_accounting.paid')}</span>
                        </div>
                    ...
                    <div className="relative flex flex-col items-center p-2 min-w-[180px] bg-white border border-gray-200 rounded-lg shadow-sm">
                        <div className="px-2 py-1 bg-white shadow text-center font-medium rounded-lg mb-4 absolute top-[-15px] right-4">
                            <span className="text-xs whitespace-normal text-center ">{t('finance_accounting.owed')}</span>
                        </div>
                ...
                <div className="flex items-center gap-2 ">
                    ...
                    >
                        <strong>{t('finance_accounting.receivables.owedAmount')}</strong>
                    </Checkbox>
                    <BaseButton
            ...
            >
                    ...
                    render={(_, record: ColumnType) => {
                        return (
                            <div
                                ...
                                onClick={() =>
                                    navigate(`${AppRouters.DEBT_TRACKING}/${record.customerId}`, {
                ...
                />
                <Column
                    title={t('finance_accounting.total_arising')}
                    ellipsis
                    width={200}
                    align="right"
                ...
                />
                <Column
                    title={t('finance_accounting.paid')}
                    ellipsis
                    width={200}
                    align="right"
            ...
            <ExportModal
                ...
                checkboxLabel={t('finance_accounting.receivables.owedAmount')}
