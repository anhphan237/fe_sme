...
import { apiCreatePaymentSlip, apiGetPaymentSlipById, apiUpdatePaymentSlip } from '@/api/payment-slip.api';
...
import { PaymentSlipRequest, PaymentSlipStatus } from '@/interface/payment-slip';

const { Text } = Typography;

const AddEditPaymentSlip = () => {
    ...
    const { data: formData } = useQuery({
        queryKey: ['getDetailPaymentSlip', id],
        queryFn: async () => {
            const res = await apiGetPaymentSlipById(id || '');
            if (res?.data) {
                const temp = {
                    ...res?.data,
    ...
    const isFormDisabled = useMemo(() => {
        ...
        if (isEdit && formData?.status === PaymentSlipStatus.Paid) {
            return true;
        }
        return false;
    ...
    useEffect(() => {
        ...
        dispatch(
            setBreadCrumbs({
                [id]: t('finance_accounting.payment_slip.edit'),
            }),
        );
    }, [id]);
    ...
    const handleAddEdit = async (data: any) => {
        try {
            const params: PaymentSlipRequest = {
                ...(isEdit && formData && { id: formData.id }),
                documentDate: data.documentDate,
                status: data.status || PaymentSlipStatus.UnPaid,
                note: data.note,
                ...
                details:
                    data.details?.map((item: any, index: number) => ({
                        paymentSlipName: item.paymentSlipName,
            ...
            const res = isEdit ? await apiUpdatePaymentSlip(params) : await apiCreatePaymentSlip(params);
            ...
            if (res.succeeded) {
                ...
                if (data?.isSaveAdd) {
                    ...
                    navigate(AppRouters.PAYMENT_SLIP);
                }
            } else {
                throw res;
            }
    ...
    return (
        <div className="p-3">
            <Form
                ...
                initialValues={{
                    status: PaymentSlipStatus.UnPaid,
            ...
            >
                <div className="max-w-full grow p-4 box-border space-y-4">
                    {isEdit && formData?.status === PaymentSlipStatus.Paid && !isAdmin && (
                        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4 rounded">
                            <div className="flex items-start">
                                <ExclamationCircleOutlined className="text-yellow-400 text-lg mt-0.5" />
                                <div className="ml-3">
                                    <p className="text-sm font-medium text-yellow-800">
                                        {t('finance_accounting.payment_slip.message.paid_slip_readonly')}
                                    </p>
                                    <p className="mt-1 text-xs text-yellow-700">{t('finance_accounting.payment_slip.message.contact_admin')}</p>
                                </div>
                            </div>
                        </div>
                    )}
                    <div className="grid grid-cols-6  gap-4">
                        ...
                        <BaseSelect
                            name="status"
                            label={t('finance_accounting.payment_slip.status')}
                            placeholder={t('finance_accounting.payment_slip.status')}
                            options={[
                                { value: PaymentSlipStatus.UnPaid, label: t('finance_accounting.payment_slip.status.unpaid') },
                                { value: PaymentSlipStatus.Paid, label: t('finance_accounting.payment_slip.status.paid') },
                            ]}
                            formItemProps={{
                                required: true,
                            }}
                        ...
                        <InfiniteScrollSelect
                            name="paymentMethod"
                            queryKey={['searchPaymentFundForPaymentSlip']}
                    ...
                    <div className="col-span-6 space-y-4">
                        <div className="col-span-6 font-bold flex justify-center items-center gap-2">
                            <div className="min-w-fit">{t('finance_accounting.payment_slip.requester')}</div>
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            <InfiniteScrollSelect
                                ...
                                queryKey={['searchUsersForPaymentSlip']}
                                label={t('finance_accounting.payment_slip.requester_name')}
                                placeholder={t('finance_accounting.payment_slip.select_requester')}
                        ...
                        {(fields, { add, remove }) => {
                            const columns: ColumnsType = [
                                ...
                                },
                                {
                                    ...
                                    dataIndex: 'paymentSlipName',
                                    width: 200,
                                    render: (_: any, __: any, index: number) => (
                                        <BaseTextArea
                                            {...fields[index]}
                                            name={[fields[index].name, 'paymentSlipName']}
                                            label=""
                                ...
                                },
                                {
                                    title: (
                                        <span>
                                            {t('finance_accounting.payment_slip.payment_date')}
                                            <span className="text-red-600">*</span>
                                        </span>
                                    ),
                                    ...
                                    render: (_: any, __: any, index: number) => {
                                        return (
                                            <BaseDatePicker
                                                ...
                                                label=""
                                                placeholder={t('finance_accounting.payment_slip.payment_date')}
                                                format="DD/MM/YYYY"
                                                formItemProps={{
                                                    ...convertedDateProps,
                                                    rules: [{ required: true, message: t('global.message.required_field') }],
                            ...
                            return (
                                <div className="col-span-6 space-y-4">
                                    <div className="col-span-6 font-bold flex justify-center items-center gap-2">
                                        <div className="min-w-fit">{t('finance_accounting.payment_slip.details')}</div>
                                        <div className="h-[1px] w-full bg-gray-400 mt-1" />
                ...
                <div className="sticky bottom-0 px-4 py-2 left-0 bg-white z-10 border-t border-gray-200 flex flex-col space-y-2">
                    <div className="flex justify-end items-center">
                        <Text className="text-lg font-semibold mr-2">{t('finance_accounting.payment_slip.totalAmount')}: </Text>
...
export default AddEditPaymentSlip;
