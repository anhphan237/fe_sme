import { apiDeleteDebtTransaction, apiSearchDebtTransactions } from '@/api/debt-receivables.api';
...
import DebtTransactionTable from '@/components/table';
...
const { Column } = DebtTransactionTable;
...
const DebtTransaction = () => {
    ...
    const { data, refetch, isFetching } = useQuery({
        ...
        queryFn: async () => {
            const res = await apiSearchDebtTransactions(filter, { loading: false });
            return res;
        },
    ...
    useEffect(() => {
        ...
        dispatch(
            setBreadCrumbs({
                [debtDocumentId]: customerName ? ` ${customerName}` : t('finance_accounting.debt_receivable'),
                transaction: t('finance_accounting.debt_transaction'),
            }),
        );
    ...
    const handleDelete = async (record: IDebtTransaction) => {
        const res = await apiDeleteDebtTransaction(record.id);
        if (res.succeeded) {
    ...
    return (
        <div className="h-full flex flex-col p-4">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {documentNumber && (
                        <span className="text-base font-medium">
                            <span className="text-gray-600">{t('sales.invoice_number')}:</span>{' '}
            ...
            <div className="flex justify-end gap-12 text-lg p-2">
                ...
                <span>
                    {`${t('finance_accounting.collected')}: `}
                    <strong
                        className={clsx({
            ...
            <DebtTransactionTable<IDebtTransaction>
