...
import { apiDeleteDebtReceivableOther, apiSearchDebtReceivableOther } from '@/api/debt-receivable-other.api';
...
import DebtReceivableOtherTable from '@/components/table';
...
import { DebtReceivableOtherRequester, DebtReceivableOtherType } from '@/interface/debt-receivable-other';

const { Column } = DebtReceivableOtherTable;

const DebtReceivableOther = () => {
    const { t } = useLocale();
    ...
    const canDelete = (record: DebtReceivableOtherType) => {
    ...
    const { data, refetch } = useQuery({
        queryKey: ['getListDebtReceivableOther', filter],
        queryFn: async () => {
            const res = await apiSearchDebtReceivableOther(filter);
            return res;
        },
    });
    ...
    const handleDelete = async (recordId: string) => {
        try {
            const res = await apiDeleteDebtReceivableOther(recordId);
            if (res.succeeded) {
    ...
    const statusOptions = [
        { label: t('finance_accounting.debt_receivable_other.status.paid'), value: COMMON_STATUS.Paid },
        { label: t('finance_accounting.debt_receivable_other.status.unpaid'), value: COMMON_STATUS.UnPaid },
    ];
    ...
    const exportFilters: AdditionalFilterConfig[] = [
        {
            ...
            queryKey: ['debtReceivableOtherExpensesTypeExport'],
            showSearch: true,
            allowClear: true,
            mode: 'multiple',
        },
    ...
    return (
        <div className="h-full flex flex-col p-4">
            <div className="flex justify-between flex-wrap mx-2 mb-2 gap-2">
                <div className="flex gap-2">
                    <BaseButton
                        ...
                        onClick={() => navigate(AppRouters.DEBT_RECEIVABELES_OTHER_ADD)}
                ...
                <div className="flex gap-2 justify-end">
                    ...
                    <InfiniteScrollSelect
                        name="ticketType"
                        queryKey={['debtReceivableOtherExpensesTypeFilter']}
                        placeholder={t('finance_accounting.payment_slip.ticket_type.income')}
                        mode="multiple"
            ...
            <DebtReceivableOtherTable<DebtReceivableOtherType>
            ...
            >
                ...
                <Column
                    ...
                    align="center"
                    render={(_, record: DebtReceivableOtherType) => {
                        return (
                            <span
                                className="text-blue-500 font-medium cursor-pointer"
                                onClick={() => navigate(`${AppRouters.DEBT_RECEIVABELES_OTHER}/${record.id}`)}
                            >
                                {record.ticketNumber || '-'}
                            </span>
                        );
                ...
                />
                <Column
                    title={t('finance_accounting.payment_slip.ticket_type.income')}
                ...
                <Column
                    title={t('finance_accounting.debt_receivable_other.responsible_officer_short')}
                    dataIndex="requester"
                    align="left"
                    key="requester"
                    onCell={() => ({ width: 180 })}
                    onHeaderCell={() => ({ style: { width: 180, minWidth: 180 } })}
                    render={(requester: DebtReceivableOtherRequester) => {
                        return <span className="font-medium">{requester?.userName || '-'}</span>;
                    }}
                />

                <Column
                    title={t('finance_accounting.debt_receivable_other.total_collection')}
                                ...
                                <Popconfirm
                                    ...
                                    description={
                                        record.status === COMMON_STATUS.Paid && !isAdmin
                                            ? t('finance_accounting.debt_receivable_other.message.paid_admin_only')
                                            : undefined
                                    }
                ...
                />
            </DebtReceivableOtherTable>
            <ExportModal
                ref={exportRef}
                submitLabel={t('global.download')}
                title={t('export.title')}
                urlPath="debt-receivable-other"
...
export default DebtReceivableOther;
