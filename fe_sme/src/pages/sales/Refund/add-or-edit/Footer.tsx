...
import { InputHelper, formatMoney } from '@/utils/helpers';
...
const AddOrEditFooter = ({ initialStatus }: { initialStatus?: number }) => {
    ...
    const discount = Form.useWatch('discountAmount') || 0;
    const previousDebt = Form.useWatch('previousDebt') || 0;
    ...
    const summary = useMemo(() => {
        return products.reduce(
            (acc: any, item: any) => {
                ...
                acc.totalAmount += sellingPriceByItem(item, 'sale') || 0;

                if (returnQuantity > 0) {
                    acc.totalReturnQuantity += returnQuantity;
                    acc.refundTotal += sellingPriceByItem({ ...item, quantity: returnQuantity }, 'sale') || 0;
                }
    ...
    const invoiceTotal = totalAmount + deliveryFee - discount + previousDebt;
    ...
    return (
        ...
        >
            <Col span={4}>
                <SummaryItem
                    label={t('sales.total_product.invoice')}
            ...
            <Col span={4}>
                <SummaryItem label={t('sales.total_price.sale')} value={formatMoney(invoiceTotal)} color="text-blue-600" />
            </Col>

            <Col span={4}>
                <SummaryItem
                    label={t('sales.total_product.refund')}
            ...
            <Col span={4}>
                <SummaryItem label={t('sales.total_refund')} value={formatMoney(refundTotal)} color="text-red-600" />
            </Col>
