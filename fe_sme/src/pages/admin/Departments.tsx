...
import { useDepartmentsQuery, useUsersQuery } from '../../hooks/queries'
import { createDepartment, updateDepartment } from '../../shared/api/departments'
...
function Departments() {
  ...
  const [createManagerDropdownOpen, setCreateManagerDropdownOpen] = useState(false)
  ...
  const [assignManagerError, setAssignManagerError] = useState<string | null>(null)
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleCreateSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-[#1d1d1f]">Name</label>
            <input
              type="text"
              value={createName}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-[#1d1d1f]">Type</label>
            <select
              value={createType}
        ...
        {editDept && (
          <form onSubmit={handleUpdateSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" variant="primary" className="rounded-lg" disabled={updating}>
      ...
      >
        {assignManagerDept && (
          <form onSubmit={handleAssignManagerSubmit} className="space-y-4">
            {assignManagerError && (
              <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{assignManagerError}</p>
            )}
            <p className="text-sm text-muted">
              Assign a manager for <strong>{assignManagerDept.name}</strong>.
            </p>
            <div>
              <label className="mb-1 block text-sm font-medium text-[#1d1d1f]">Manager</label>
              <select
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" variant="primary" className="rounded-lg" disabled={assignManagerUpdating}>
            ...
            <tbody>
              {data?.map((dept) => (
                <tr key={dept.id} className="border-t border-stroke hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-muted">{managerName(dept.managerUserId)}</td>
