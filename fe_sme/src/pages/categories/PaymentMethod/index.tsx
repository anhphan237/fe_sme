import { apiAddPaymentMethod, apiDeletePaymentMethod, apiSearchPaymentMethod, apiUpdatePaymentMethod } from '@/api/category.api';
...
import PaymentMethodTable from '@/components/table';
...
import { IAddEditPaymentMethod } from '@/interface/category';
import { PageFilter } from '@/interface/common';

const { Column } = PaymentMethodTable;
...
const PaymentMethod = () => {
    const [formData, setFormData] = useState<IAddEditPaymentMethod | null>(null);
    const [filter, setFilter] = useState<PageFilter>({
        pageSize: 10,
        pageNumber: 1,
        search: undefined,
    });
    const isFullPermission = useCheckIsFullPermission(DefaultMappingPermission[AppRouters.PAYMENT_METHOD]);
    const { t } = useLocale();

    const { data, refetch } = useQuery({
        queryKey: ['getListPaymentMethod', filter],
        queryFn: async () => {
            const res = await apiSearchPaymentMethod(filter);
            return res;
        },
    });

    const handleAddEdit = async (data: IAddEditPaymentMethod) => {
        const isEdit = !!formData?.id;
        const params = isEdit ? { id: formData?.id, name: data?.name, code: formData?.code } : data;
        const res = isEdit ? await apiUpdatePaymentMethod(params) : await apiAddPaymentMethod(params);
        if (res.succeeded) {
    ...
    const handleDelete = async (recordId: string) => {
        const res = await apiDeletePaymentMethod(recordId);
        if (res.succeeded) {
    ...
    return (
        <div className="h-full flex flex-col p-4">
            ...
            <BaseModal
                title={t(formData?.id ? 'Update payment method' : 'Create payment method')}
                open={!!formData}
            ...
            >
                ...
                >
                    <Form.Item
                        ...
                        rules={[{ required: true, message: t('Payment method name cannot be empty') }]}
                        required={true}
                        name={'name'}
                    >
                        <Input placeholder={t('Payment method name')} />
                    </Form.Item>
