import { apiAddPartnerType, apiDeletePartnerType, apiSearchPartnerType, apiUpdatePartnerType } from '@/api/category.api';
...
import PartnerTypeTable from '@/components/table';
...
import { IAddEditPartnerType } from '@/interface/category';
import { PageFilter } from '@/interface/common';

const { Column } = PartnerTypeTable;
...
const PartnerType = () => {
    const [formData, setFormData] = useState<IAddEditPartnerType | null>(null);
    const [filter, setFilter] = useState<PageFilter>({
        pageSize: 10,
        pageNumber: 1,
        search: undefined,
    });
    const isFullPermission = useCheckIsFullPermission(DefaultMappingPermission[AppRouters.PARTNER_TYPE]);
    const { t } = useLocale();

    const { data, refetch } = useQuery({
        queryKey: ['getListPartnerType', filter],
        queryFn: async () => {
            const res = await apiSearchPartnerType(filter);
            return res;
        },
    });

    const handleAddEdit = async (data: IAddEditPartnerType) => {
        const isEdit = !!formData?.id;
        const params = isEdit ? { id: formData?.id, name: data?.name, code: formData?.code } : data;
        const res = isEdit ? await apiUpdatePartnerType(params) : await apiAddPartnerType(params);
        if (res.succeeded) {
    ...
    const handleDelete = async (recordId: string) => {
        const res = await apiDeletePartnerType(recordId);
        if (res.succeeded) {
    ...
    return (
        <div className="h-full flex flex-col p-4">
            ...
            <BaseModal
                title={t(formData?.id ? 'partner.edit.popup.header' : 'partner.add.popup.header')}
                open={!!formData}
            ...
            >
                ...
                >
                    <Form.Item
                        ...
                        rules={[{ required: true, message: t('partner.name.required') }]}
                        required={true}
                        name={'name'}
                    >
                        <Input placeholder={t('partner.name')} />
                    </Form.Item>
