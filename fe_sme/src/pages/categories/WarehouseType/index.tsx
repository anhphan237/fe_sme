import { apiAddWarehouseArea, apiDeleteWarehouseArea, apiSearchWarehouseArea, apiUpdateWarehouseArea } from '@/api/category.api';
...
import WarehouseAreaTable from '@/components/table';
import { notify } from '@/components/toast-message';

import { formatDateTime } from '@/utils/format-datetime';

import { IAddEditWarehouseArea } from '@/interface/category';
import { PageFilter } from '@/interface/common';

const { Column } = WarehouseAreaTable;
...
const WarehouseArea = () => {
    const [formData, setFormData] = useState<IAddEditWarehouseArea | null>(null);
    ...
    const { data, refetch } = useQuery({
        queryKey: ['getListWarehouseArea', filter],
        queryFn: async () => {
            const res = await apiSearchWarehouseArea(filter);
            return res;
        },
    });

    const handleAddEdit = async (data: IAddEditWarehouseArea) => {
        const isEdit = !!formData?.id;
        const params = isEdit ? { id: formData?.id, name: data?.name, code: formData?.code } : data;
        const res = isEdit ? await apiUpdateWarehouseArea(params) : await apiAddWarehouseArea(params);
        if (res.succeeded) {
    ...
    const handleDelete = async (recordId: string) => {
        const res = await apiDeleteWarehouseArea(recordId);
        if (res.succeeded) {
    ...
    return (
        <div className="h-full flex flex-col p-4">
            ...
            <BaseModal
                title={t(formData?.id ? 'Update warehouse type' : 'Create warehouse type')}
                open={!!formData}
            ...
            >
                ...
                >
                    <Form.Item
                        ...
                        name={'name'}
                    >
                        <Input placeholder={t('Warehouse type name')} />
                    </Form.Item>
