import { apiAddDepartment, apiDeleteDepartment, apiSearchDepartment, apiUpdateDepartment } from '@/api/category.api';
...
import { IAddEditDepartment } from '@/interface/category';
import { PageFilter } from '@/interface/common';

const { Column } = DepartmentTable;
...
const Department = () => {
    const [formData, setFormData] = useState<IAddEditDepartment | null>(null);
    const [filter, setFilter] = useState<PageFilter>({
        pageSize: 10,
        pageNumber: 1,
        search: undefined,
    });
    const isFullPermission = useCheckIsFullPermission(DefaultMappingPermission[AppRouters.DEPARTMENT]);
    const { t } = useLocale();

    const { data, refetch } = useQuery({
        queryKey: ['getListDepartment', filter],
        queryFn: async () => {
            const res = await apiSearchDepartment(filter);
            return res;
        },
    });

    const handleAddEdit = async (data: IAddEditDepartment) => {
        const isEdit = !!formData?.id;
        const params = isEdit ? { id: formData?.id, name: data?.name, code: formData?.code } : data;
        const res = isEdit ? await apiUpdateDepartment(params) : await apiAddDepartment(params);
        if (res.succeeded) {
    ...
    const handleDelete = async (recordId: string) => {
        const res = await apiDeleteDepartment(recordId);
        if (res.succeeded) {
    ...
    return (
        <div className="h-full flex flex-col p-4">
            ...
            <BaseModal
                title={t(formData?.id ? 'Update department' : 'Create department')}
                open={!!formData}
