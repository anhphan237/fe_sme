import { apiAddCustomerType, apiDeleteCustomerType, apiSearchCustomerType, apiUpdateCustomerType } from '@/api/category.api';
...
import { IAddEditCustomerType } from '@/interface/category';
import { PageFilter } from '@/interface/common';

const { Column } = CustomerTypeTable;
...
const CustomerType = () => {
    const [formData, setFormData] = useState<IAddEditCustomerType | null>(null);
    const [filter, setFilter] = useState<PageFilter>({
        pageSize: 10,
        pageNumber: 1,
        search: undefined,
    });
    const isFullPermission = useCheckIsFullPermission(DefaultMappingPermission[AppRouters.CUSTOMER_TYPE]);
    const { t } = useLocale();

    const { data, refetch } = useQuery({
        queryKey: ['getListCustomerType', filter],
        queryFn: async () => {
            const res = await apiSearchCustomerType(filter);
            return res;
        },
    });

    const handleAddEdit = async (data: IAddEditCustomerType) => {
        const isEdit = !!formData?.id;
        const params = isEdit ? { id: formData?.id, name: data?.name, code: formData?.code } : data;
        const res = isEdit ? await apiUpdateCustomerType(params) : await apiAddCustomerType(params);
        if (res.succeeded) {
    ...
    const handleDelete = async (recordId: string) => {
        const res = await apiDeleteCustomerType(recordId);
        if (res.succeeded) {
    ...
    return (
        <div className="h-full flex flex-col p-4">
            ...
            <BaseModal
                title={t(formData?.id ? 'Update customer type' : 'Create customer type')}
                open={!!formData}
            ...
            >
                ...
                >
                    <Form.Item
                        label={t('Customer type name')}
                        rules={[{ required: true, message: t('Customer type name cannot be empty') }]}
                        required={true}
                        name={'name'}
                    >
                        <Input placeholder={t('Customer type name')} />
                    </Form.Item>
            ...
            >
                ...
                <Column
                    ...
                    dataIndex="code"
                    onCell={() => ({ width: 220 })}
                    onHeaderCell={() => ({ style: { width: 220, minWidth: 220 } })}
                    ellipsis
                    key="code"
